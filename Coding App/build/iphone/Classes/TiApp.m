...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kHicks_Anthony_Coding_AppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kHicks_Anthony_Coding_AppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Hicks_Anthony_Coding_App app using the node.js CLI
