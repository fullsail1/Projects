...
NSString * Hicks_Anthony_Coding_App$ModuleRequireFormat = @"(function(exports){"
...
@implementation Hicks_Anthony_Coding_AppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_hicks_anthony_coding_app);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Hicks_Anthony_Coding_App global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Hicks_Anthony_Coding_App" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hicks_anthony_coding_app = [[Hicks_Anthony_Coding_AppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hicks_anthony_coding_app];
	
	NSString *_hicks_anthony_coding_appNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hicks_anthony_coding_appNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hicks_anthony_coding_app valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Hicks_Anthony_Coding_App$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hicks_anthony_coding_app.kroll" 
